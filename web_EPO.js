...
var CORS_OPTIONS = {
  ...
  methods: 'POST, OPTIONS',                     // configure Access-Control-Allow-Methods header
  ...
  maxAge: 3600                                  // client can cache the pre-flight request results for 3600 sec
}
...
app.options('/epoapi/biblio', cors(CORS_OPTIONS));   // respond to  CORS pre-flight request from client (an HTTP OPTIONS request)
...
app.post('/epoapi/biblio', cors(CORS_OPTIONS), function(clientReq, serverResp) {      // respond to CORS POST request from client
  ...
  // if POST request has a session cookie (generated by the main site server) and a client cookie then send the
  // request to the EPO API; do not validate the cookies, just verify that they have been sent.
...
// check for presence of a sessionCookie and a clientCookie, but do nothing to check that the cookies were actually
...
// are not valid.
  ...
  if (sValue) {
    ...
    invalidCookies("In verifyEPOReqCookies, no session cookie");
  }
}
