...
// clientCookie be set.
  ...
  function checkRemove() {
    if (this.count <= 0) {                               // remove cookie if no cookie jobs have been added in the meantime
      ...
      debug("In cookieTracker, deleted a cookie for total count of " + this.count);
    }
    else {
...
function on_main_page_load() {
  ...
  function on_jsverify_complete(data) {
  // callback for the $.get request for the 'yappee_cl' cookie; this cookie is set in js for each request for Google data
  ...
  // the 'yappee_cl' cookie will not exist in the browser.
