...
// map a patent number entered by the user in the input box at the top of the force map; called from on_click_btn.
  ...
  // patent_url must be a valid Google patent url for a patent in the Google database (use this.validatePatent).
    ...
    $.when(patentTab(patent_url, patentLabel))                 // create the patent tab
              .done(on_patent_tab_done)
              .fail(on_patent_tab_fail);

    function on_patent_tab_done(patent_no) {
    // callback for $.when in this.getPatent if patentTab is successful creating the patent tab;
    // patent_no parsed from the Google patent document
      taskTracker.finishTask(1);
      toggleButton(patent_no, "btn-map-add", "add");
      addMapPatent(patent_no, "map", onMapComplete);
    }

    function on_patent_tab_fail() {
    // callback for $.when in this.getPatent if there is an error creating the patent tab.
      debug.log("In randomPatentPicker.mapRandomPatent, failed to create tab for patent " + patentLabel);
    }
  ...
  // generate a random US patent number and map it; call optional callback when done
    ...
    // try again with a different random patent number.
      ...
      if (!respObject.error) {
        ...
        $.when(patentTab(patent_url, patentLabel))
              .done(on_patent_tab_done)
              .fail(on_patent_tab_fail);
      }
      else {
    ...
    function on_patent_tab_done(patent_no) {
    // callback for $.when in on_check_patent_url_complete if patentTab is successful creating the patent tab;
    // patent_no parsed from the Google patent document
      taskTracker.finishTask(1);
      toggleButton(patent_no, "btn-map-add", "add");
      addMapPatent(patent_no, "map", onMapComplete);
    ...
    function on_patent_tab_fail() {
    // callback for $.when in on_check_patent_url_complete if there is an error creating the patent tab.
      debug.log("In randomPatentPicker.mapRandomPatent, failed to create tab for patent " + USpatent);
    }
