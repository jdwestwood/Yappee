...
// from removePatentFromMapped (user clicks unmap button), nextDeletions is null.
  ...
  if (iL >= 0) {
    taskTracker.initialize("Deleting patents", undefined);
...
// on_click_undo_delete_btn, and restoreClearedPatents
  ...
  // mapExistingTargetPatents after timeout when undeleting a source patent
    ...
    if (patentLists["favorites"].indexOf(patent_no) != -1) {     // show delete button in Favorites list entry
...
// plot reference patents for which epoBiblio data has already been queried and processed
  ...
  if (groupList.length > 0) {
    var iL = 0;
    taskTracker.initialize("Fetching patents", undefined);
    taskTracker.startTask(groupList.length);
    plotTargetGroup();
  }
  else {
...
function TaskTracker() {
  this.initialize = function(taskType, callback) {
    ...
    this.error = false;
    debug("In taskTracker, initializing task " + taskType);
  }
  this.startTask = function(num) {
    if (!this.error) {
      ...
      debug("In taskTracker, starting " + num + " tasks");
    }
  }
  this.finishTask = function(num) {
    if (!this.error) {
      if (this.doneCount + num <= this.maxCount) {
        this.doneCount += num;
      }
      else {
        this.doneCount = this.maxCount;
        debug("In taskTracker.finishTask, warning: more tasks finishing than were started");
      }
      updateMessage.call(this);
      updateTimeout.call(this); 
      debug("In taskTracker, finished " + num + " task; " + this.doneCount + "/" + this.maxCount + " complete");
    }
  }
  ...
  function updateTimeout() {
    ...
    this.runningTimeout = window.setTimeout(tasksComplete.bind(this), newTime);
  }
  ...
  function updateMessage() {
    $("span.status-line-queries").text(report.call(this));
  }

  function report() {
    return " " + this.doneCount + "/" + this.maxCount + " tasks done";
  }
  ...
  this.reportError = function(errorMessage) {
    ...
    $("span.status-line-type").text(errorMessage);
    $("span.status-line-queries").text("");
    $("div.status-line").addClass("make-visible");
  }
}
