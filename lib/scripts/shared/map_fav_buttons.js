...
function setupMapFavButtons($btnContext, $docContext, page_type, patent_no) {
  ...
  (patentLists["map"].indexOf(patent_no) != -1) ? btnMap.addClass("show-result-btn") : btnMap.removeClass("show-result-btn");
  (patentLists["favorites"].indexOf(patent_no) != -1) ? btnFav.addClass("show-result-btn") : btnFav.removeClass("show-result-btn");
}
...
// Get the metadata (info in the panel on the right side of a related patents page.
  cookieTracker.start()                                     // set clientCookie
  $.ajax({ type: "POST",
...
// target patents to force map; call the callback on_mapping_complete when done.
  if (patentLists["map"].indexOf(patent_no) == -1) {       // if patent not already mapped
    ...
    if (!(patent_no in patentBiblio)) {                    // if no bibliographic info
      ...
      // Get biblio data from the Google patent doc for patent_no and get the list of Google related patents for patent_no
      // Use jQuery deferred objects to run the two tasks in parallel; on_patent_data_done is the callback when
      // successful; on_patent_data_fail is callback for error.
      taskTracker.initialize("Fetching patent", undefined);
      taskTracker.startTask(3);                            // start three query tasks
      cookieTracker.start();                               // set clientCookie, so any requests to the server will be valid
      $.when(getPatentBiblioData(), getRelatedPatentList())
            .done(on_patent_data_done)
            .fail(on_patent_data_fail);
    }
    else {
  ...
  function on_patent_data_done() {
  // callback for $.when after getPatentBiblioData and getRelatedPatentList have completed successfully;
  // patentBiblio now has a complete entry for patent_no (unless queries took too much time).
    debug("In on_patent_data_done, added entry for " + patent_no + " to patentBibio");
    cookieTracker.end(1000);
    window.setTimeout(mapSourcePatent, 500);  // short delay to allow taskTracker status update on force map
  }

  function on_patent_data_fail() {
  // callback for $.when if getPatentBiblioData and getRelatedPatentList have not completed successfully.
    cookieTracker.end(0);
    debug("Error getting data in getPatentBiblioData or getRelatedPatentList.");
  }

  function getPatentBiblioData() {
  // set up a jQuery deferred object for getting biblio data and referenced patents from a Google patent document
    var patentBiblioDataDeferred = $.Deferred();         // construct the deferred object
    var patentDocHTML = getPatentDocHTML(patent_no);     // get the Google patent document HTML if it exists
    if (patentDocHTML) {
      processPatentPage(patentDocHTML);                  // parse the document for biblio data and patent reference lists
    }
    else {                                               // make a $.get request for the document; processPatentPage is the
      getGooglePatentHTML(patent_no, processPatentPage); // callback to parse the HTML for biblio data and reference lists
    }
    return patentBiblioDataDeferred.promise();
    ...
    // called from getPatentBiblioData; return the HTML document for patent patent_no (central patent on the
    ...
    // citing patents; add the data to the patentObj object for the patent; patent_no is in the scope of the containing
    // function on_click_map_buttons.
      ...
      var file_date = $("table.patent-bibdata td.patent-bibdata-heading:contains('Filing date') + td.single-patent-bibdata",
                        $docBody).text();
      var pub_date = $("table.patent-bibdata td.patent-bibdata-heading:contains('Publication date') + td.single-patent-bibdata",
                        $docBody).text();
      ...
      taskTracker.finishTask(1);
      patentBiblioDataDeferred.resolve();                        // task is done: resolve the deferred object
      debug("In getPatentBiblioData: finished parsing biblio data from Google patent document for patent " + patent_no + ".");
    }
  }
...
*/
  ...
  // Looked at Network requests in chrome developer tools and noticed that the metadata (info in the panel
  // on the right side of a related patents page) is fetched using POST requests to /patents/related/rpc;
  // a very convenient way to get the related patent numbers!!
  //
  // Use chrome extension 'Postman' to debug the POST query; get 
    // set up a jQuery deferred object for getting the Google related patents.
    var relatedPatentListDeferred = $.Deferred();             // construct the deferred object
    $.ajax({ type: "POST",
             url: "/patents/related/rpc",
             headers: {"XmlHttpRequest": "2"},
             contentType: "application/x-www-form-urlencoded; charset=UTF-8",    // this is default
    ...
    return relatedPatentListDeferred.promise();
    ...
    // patent_no is in scope of containing function on_click_map_buttons 
      debug("In on_complete_related_metadata_post, metadata returned from /related/rpc for patent "
             + patent_no + " is ", data);
      if (data[10] && data[10][1]) {                       // old patents do not always have search terms included
        ...
        taskTracker.finishTask(1);
        relatedPatentListDeferred.resolve();               // complete task successfully, but with no related patents
      }
    }
    ...
    // extract the list of the related patents from the JSON
      ...
      if (data[5]) {
        ...
        taskTracker.finishTask(1);
        relatedPatentListDeferred.resolve();                        // task is done: resolve the deferred object
        debug("In getRelatedPatentList: finished querying /related/rpc for Google related patents for patent " + patent_no + ".");
      }
      else {
        taskTracker.finishTask(1);
        relatedPatentListDeferred.resolve();               // complete task successfully, but with no related patents
  ...
  // called from on_patent_data_done and on_click_map_buttons; finish mapping the source patent (patent
  ...
  // get the biblio data; process the data; and map the patents
    ...
    function mapNewTargetPatents() {
      ...
      if (epoQueryLists.length > 0) {
        taskTracker.initialize("Fetching patents", on_mapping_complete);
      ...
      // patents (cited, citing, or related), and "done", which tracks whether the EPO query has completed.
         ...
         // query the EPO API; on_EPO_query_success is the callback for getEPOPatentData
         // stagger the requests to avoid possible multiple calls to cache for the epoCookie value.
         window.setTimeout(getEPOPatentData.bind(this, epoPatList, on_EPO_query_success.bind(undefined, iList)), iList*50);
      }

      function on_EPO_query_success(iList, data) {
      // called from epoQueryLists if EPO query is successful and returns 'data' in JSON; parse the JSON for biblio info
      // patent_no, epoQueryLists, iList are defined in the containing scope; iList is passed via .bind; data is passed
      // from the successful ajax query.
        var processedList = parseEPOPatentData(data);
        if (processedList) {
          addEPOPatentDataToForceMap(patent_no, processedList, epoQueryLists, iList);
          debug("In mapNewTargetPatents, adding queried EPO data for EPO query list " + (iList+1) + " with " +
                   processedList.length + " " +  epoQueryLists[iList]["type"] + " patents to the force map");
          updateForceMap();
        }
        taskTracker.finishTask(1);
      }
