...
// on_search_result_load, on_related_load, and on_patent_load.
  debug("In process_links, processing links for a " + page_type + " page");
        ...
        switch (link_type) {
          case "/patents/":
            ...
            if (strip_url.search('/patents/related') >= 0) {
              ...
              // patent link on search, related, or patent page, except for Grant, Application, and Also Published As which
          ...
          case "/url?":
            if (strip_url.search('/www.google.com/patents/') >= 0) {
              ...
              // this branch should never be called since the 'related' link on a search page
              // is processed when the page loads; after mousedown, url changes, starts with /url?
              if (page_type == "search") {
                $(this).on("click", {$context: $context, page: page_type}, on_click_new_related_url);
              }
              else {
                debug("In process_links, found unexpected type of '/url?' link on a " + page_type + " page");
              }
            }
            break;
          default:
...
function on_click_new_patent_url(event) {
  // patent url's can be clicked on map, search, related (as a related patent, the root patent, or a link in the Metadata
  // section), or patent pages event.data.$context stores the jQuery context of the element that triggered the event
...
// page_type ("mapped-list", "favorites-list",  "map", "main" "search", "patent", or "related")
  ...
  switch (page_type) {
    ...
    case "main":
      ...
      break;
    case "patent": case "metadata-patent":
