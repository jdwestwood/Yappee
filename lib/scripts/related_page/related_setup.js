...
function make_related_tab(related_url) {
  ...
  function on_related_load(event) {
    ...
    // see http://stackoverflow.com/questions/6997826/alternative-to-domnodeinserted; specify animation during of 0.5 sec in
    // jdwpriorartpage.css and use the animationend event to give Google scripts time to run; in the Metadata section, a
    // script seems to add the 'Discuss this patent' button after the section has loaded; in Chrome, it seems to be already
    // there when the div.discuss-this-parent is removed in my script, but in Firefox, the script seems to add it after the
    // div has been deleted and it somehow ends up as the last tag in the <body>
    $("body", $context).addClass("make-invisible");          // so user does not see the web page manipulations
    $("td.content-td", $context)
      .on('webkitAnimationEnd', "div.r", on_divr_added);                                // Chrome animation event
    $("td.content-td", $context)
      .on('animationend', "div.r", on_divr_added);                                      // Firefox animation event
    $("td.metadata-td", $context)                                                         // Chrome animation event
      .on('webkitAnimationEnd', "div.metadata", {"tab_id": tab_id, "$context": $context}, on_divmeta_added);
    $("td.metadata-td", $context)                                                         // Firefox animation event
      .on('animationend', "div.metadata", {"tab_id": tab_id, "$context": $context}, on_divmeta_added);

//    calling process_links here and when each div.r is added sometimes causes two event handlers to be attached to
//    each patent hyperlink in Firefox, but Chrome seems to work with process_links in both places, presumably
//    because Chrome does not find the div.r's present at this point.
//    process_links($context, "related");
  ...
  // called for each div.r that is added; divr is 'this' for the event (a jQuery delegated event).
    var $divr = $(this);
    // need to make sure that this event handler is called just once; Firefox triggers the animation event each time the
    // user clicks on the related tab, but Chrome does not.
    if ($divr.hasClass("animation-done")) {
      return;
    }
    else {
      $divr.addClass("animation-done");
      var $divrContext = $divr.contents();
  ...
  // the divmeta div; the page_type is 'patent' - the root patent of the prior art search
    var $divmeta = $(this);
    // need to make sure that this event handler is called just once; Firefox triggers the animation event each time the
    // user clicks on the related tab, but Chrome does not.
    if ($divmeta.hasClass("animation-done")) {
      return;
    }
    else {
      $divmeta.addClass("animation-done");
      var $context = event.data.$context,
          tab_id = event.data.tab_id;
      process_links($divmeta, "metadata-patent");
      // the 'Discuss the patent' button ('patent' misspelled as 'parent' in the div id!)
      $("div#discuss-this-parent, div#discuss-this-patent", $context).remove();
      if ($("div.metadata h2 span.hide-result-btn", $context).length == 0) {      // if 'map' and 'favorite' buttons are not present
        // put all css floated buttons before the patent link so they are rendered first and stay on the same line;
        ...
        var patent_no = getPatentURL_No(btnMap, $context, "related-root").patent_no;
        setupMapFavButtons($divmeta, $context, "related-root", patent_no);
      }
      else {
        debug("In on_divmeta_added, was not expecting to have this event handler called twice");
      }
