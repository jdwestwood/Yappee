...
// on_patent_tab_complete when done
  ...
  // patent document at patent_url.
    ...
    // modify the HTML of the Google patent page after it loads
      ...
      // Attach indices to the 'Application', 'Grant' buttons because their can be more than one 'Application' button
      // and event handler needs to infer which one was clicked.
      $("div.viewport-chrome-toolbar div:contains('Application')", $context)
        .each( function(index) {$(this).attr("data-index", index);});        // Application button(s)
      $("div.viewport-chrome-toolbar div:contains('Grant')", $context)
        .each( function(index) {$(this).attr("data-index", index);});        // Grant button
      // Attach special click event handler 'Also published as' button so it opens in the iframe.
      ...
      // of my own so buttons function correctly.
        ...
        var origLeftToolbar = $("div#left-toolbar-buttons div.viewport-chrome-toolbar", $context);
        ...
        origLeftToolbar.replaceWith(origLeftToolbar.clone('false'));
        var newLeftToolbar = $("div#left-toolbar-buttons div.viewport-chrome-toolbar", $context);
        newLeftToolbar.on('mousedown', divMouseDown);
        newLeftToolbar.on('click', on_click_left_patent_div_button);
        ...
        newRightToolbar.on('click', on_click_right_patent_div_button);

        function on_click_right_patent_div_button(event) {
        // so the 'Find prior art', 'View PDF', and 'Download PDF' buttons work correctly
          switch ($(event.target).text()) {
            ...
            case "Download PDF":
              ...
              break;
            default:
              debug("Unexpected button clicked on the right toolbar of patent tab for " + patent_no);
          }
        }
      }

      function on_click_left_patent_div_button(event) {
      // so the 'Grant' and 'Application' buttons work correctly in both Chrome and Firefox; original Google behavior is
      // to trigger on the mousedown and mouseup events; a simple click handler works in Chrome, but in Firefox, the
      // navigation has already started by the time my click handler is triggered, so the yappee_cl cookie is never sent
      // and the replacement patent doc does not load; so to make everything work in both browsers, I get rid of all the
      // Google handlers by replacing the div#left/right-toolbar-buttons with clones, and then attach my own behavior.
        var $target = $(event.target);
        switch ($target.text()) {
          case "Application": case "Grant":
            $iframe.css("visibility", "hidden");
            cookieTracker.start();
            var $context = $(this.ownerDocument);
            // expected index of <a> element in the $a jQuery object below that corresponds to the button that user
            // clicked; a patent page can have more than one Application button, so an index is applied in
            var thisIndex = $target.attr("data-index");
            // the <a> element associated with the button (which is a div element); remove the click handler attached
            // in process_links; when user clicks on the button, user is clicking on the div, not the <a> that; Google
            // mousedown/mouseup handlers on the div originally took care of triggering the navigation to the link
            // in the associated <a> element; I removed these handlers because the behavior was screwed up in Firefox,
            // so now I need to trigger the navigation myself.
            var application_url = $("a[data-label='" + $target.text() + "']", $context).eq(thisIndex).attr("href");
            ...
            $iframe.attr("src", application_url)
            window.scrollTo(0,0);
            break;
          default:
            debug("Unexpected button clicked on the left toolbar of patent tab for " + patent_no);
        }
      }
...
//        $iframe.css("visibility", "hidden");
        cookieTracker.start();
        var $context = $(this.ownerDocument);
console.log(document.cookie, $context.get(0).cookie);
      ...
      // triggered when the iframe loads with new data.
        ...
        cookieTracker.start();
        var $context = $(this.ownerDocument);
console.log(document.cookies, $context.cookies);
